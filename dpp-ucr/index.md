...
- **Product Identity**: The product identity of a DPP equals the subjectID of a VC which needs to be globally unique. After the inital issuing of a DPP, additional live cycle data can be added to the product, by issuing those VCs to the same  product ID. All VCs that belong to a product ID can be collected in a credentials store, e.g. as part of a digital product wallet.
- **Real-time DPP updates / amendments**: If product information changes or must be added  (e.g. State-of-Health credentials, transportantion or repair events, or End-of-Live handling credentials), a Digital Product Identity Wallet allows to receive all Verifiable Credentials for a given subject ID in real time of a credential store. That means, by issuing VCs to a product wallet, DPP updates can become visible for all authorized actors in real-time.
